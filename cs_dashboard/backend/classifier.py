...
FALLBACK_SUBS = {"내부 이력", "부재 처리", "지면학습", "교사 상담 요청", "기기 교체 요청", "기기 장기미회수"}

RULES = [
    ("해지 확정",          ["해지확정", "[해지확정]", ">해지확정", "해지로 처리요청", "해지처리 요청"]),
    ("해지 방어",          [">성공", "-성공("]),
    ("해지 상담",          ["차 상담]", "해지방어", "방어 상담", "해지 방어 상담", "해지금", "해지부서", "해지 요청", "재가입 부정", "재가입 절대 부정"]),
    ...
    ("이벤트·혜택 문의",   ["지인추천 혜택", "이벤트 문의", "감사편지", "이벤트 참여"]),
    ("윙크북스",           ["윙크북스"]),
    ("구독취소",           ["구독취소", "구독 취소"]),
...
def classify(memo: str) -> tuple:
    ...
    if len(main_groups) > 1:
        for priority_main in ("해지·유지 상담", "네트워크·앱 오류", "기기·하드웨어 오류", "미납·결제"):
